...
Dos matrices son iguales si tienen el \textbf{mismo orden} y cada una de las entradas repsectivas son iguales es decir \(A=[a_{ij}]_{n\times m}\) y \(B=[b_{ij}]_{n\times m}\) son iguales si \(a_{ij}=b_{ij}\), \(i=1,2,\ldots n\) y \(j=1,2,\ldots m\)

\hypertarget{algebra-de-matrices}{%
\section{Algebra de matrices}\label{algebra-de-matrices}}

Sean las matrices \(A=[a_{ij}]_{n\times m}\) y \(B=[b_{ij}]_{p\times q}\) entonces la suma y producto de matrices se define
...
\item
  El producto es posible si \(n=p\) es decir el número columnas de la primera matriz es igual al número de filas de la segunda matriz, entonces
  \begin{align*}
  A\cdot B&=\left[\sum_{k=1}^pa_{ik}b_{kj}\right]_{n\times m}\\
  &=\begin{pmatrix}
  \sum_{k=1}^pa_{1k}b_{k1}&\sum_{k=1}^pa_{1k}b_{k2}&\ldots&\sum_{k=1}^pa_{1k}b_{km}\\
  \sum_{k=1}^pa_{2k}b_{k1}&\sum_{k=1}^pa_{2k}b_{k2}&\ldots&\sum_{k=1}^pa_{2k}b_{km}\\
  \vdots & \vdots & \ddots &\vdots \\
  \sum_{k=1}^pa_{nk}b_{k1}&\sum_{k=1}^pa_{nk}b_{k2}&\ldots&\sum_{k=1}^pa_{nk}b_{km}\\
...
\protect\hypertarget{exm:unnamed-chunk-9}{}{\label{exm:unnamed-chunk-9} }Sean \(\begin{pmatrix} 3&-1&2\\ 2&-1&2\\ 1&-1&0\\ 5&0&0\\ \end{pmatrix}_{4\times 3}\) y \(\begin{pmatrix} 0&-1&2&2&0\\ 1&-1&-2&1&1\\ 3&-1&-3&5&2\\ \end{pmatrix}_{3\times 5}\) entonces \(A\cdot B=\begin{pmatrix} 5&-4&2&10&3\\ 5&-3&-2&1&2\\ 3&-1&-3&5&3\\ 3&-1&-3&5&2\\ \end{pmatrix}_{4\times 5}\)
